...
Autonomy is earned, not granted. Start with heavily constrained agents that do one thing reliably. Add autonomy only as you prove reliability. A 95% success rate per step means only 60% by step 10. Push for guardrails before capabilities, logging before features.

### Agent Anti-Patterns

| Anti-Pattern | Problem |
|--------------|---------|
| **Unbounded Autonomy** | Agents without hard limits on iterations, cost, or scope spiral out of control |
| **Trusting Agent Outputs** | Agent outputs must be validated against ground truth; never assume correctness |
| **General-Purpose Autonomy** | Agents that try to do everything do nothing well; specialize first |

### Reliability Sharp Edges

| Issue | Severity | Solution |
|-------|----------|----------|
| Compounding errors over many steps | Critical | Reduce step count, decompose into smaller tasks |
| Runaway costs from long agent loops | Critical | Set hard cost limits and max iterations |
| Failures at scale vs. demo success | Critical | Test at scale before production |
| Agent hallucinating tool results | High | Validate outputs against ground truth |
| Flaky tool calls to external APIs | High | Build robust API clients with retries and fallbacks |
| Over-permissioned agents | High | Apply least privilege principle |
| Context window exhaustion | Medium | Track context usage, summarize history |
| Difficulty debugging agent decisions | Medium | Implement structured logging of all decisions |

### Agent Capability Areas

- Agent loops (ReAct, Plan-Execute)
- Goal decomposition
- Self-correction and reflection patterns
