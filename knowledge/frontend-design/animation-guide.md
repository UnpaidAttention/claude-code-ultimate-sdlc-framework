...
## 10. CSS Scroll-Driven Animations (Modern Standard)

### What Are Scroll-Driven Animations?

Pure CSS animations tied to scroll position — no JavaScript needed.

```
Two types of scroll timelines:
├── scroll(): Progress based on scroll container position
│   └── Use for: progress bars, parallax backgrounds, header transforms
└── view(): Progress based on element's visibility in viewport
    └── Use for: entry/exit reveals, fade-ins, scale-ups on scroll
```
...
```css
/* Fine-tune when animation starts/ends relative to viewport */
animation-range: entry 10% cover 40%;
/*
  entry X%  = element entering viewport (0% = just appearing, 100% = fully in)
  cover X%  = element covering viewport (0% = top edge at bottom, 100% = bottom edge at top)
  exit X%   = element leaving viewport
*/
```

### When to Use CSS Scroll-Driven vs JavaScript

| Use Case | CSS Scroll-Driven | JavaScript (GSAP/Framer) |
|----------|-------------------|--------------------------|
| Simple reveals | ✅ Preferred | Overkill |
| Progress bars | ✅ Preferred | Overkill |
| Parallax | ✅ Preferred | For complex multi-layer |
| Staggered sequences | ❌ Limited | ✅ Required |
| Physics-based | ❌ Not possible | ✅ Required |
| Complex timelines | ❌ Limited | ✅ Required |
...
Browser-native page transitions — replaces complex JS router transitions.
...
```css
/* Enable for multi-page apps (MPA) — zero JS */
@view-transition {
  navigation: auto;
}
...
- ✅ Page-to-page navigation in MPAs (zero JS needed)
- ✅ SPA route transitions (cleaner than current solutions)
- ✅ Shared element transitions (list → detail morphs)
- ❌ Not for in-page micro-interactions (use regular CSS animations)

---

## 12. Animation Decision Checklist
...
- ❌ Using JS libraries for scroll reveals that CSS `animation-timeline: view()` handles natively
- ❌ Complex SPA route transition code when View Transitions API works
