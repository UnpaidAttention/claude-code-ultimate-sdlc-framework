...
A single font file containing an entire range of weights, widths, and other axes — replacing multiple static files.

```
Traditional: 6 files (Light.woff2, Regular.woff2, Medium.woff2, SemiBold.woff2, Bold.woff2, Black.woff2)
Variable:    1 file  (Font-Variable.woff2) containing ALL weights 100-900+

Benefits:
├── Single HTTP request (faster load)
├── Infinite weight precision (font-weight: 450)
├── Animatable properties (weight transitions on hover)
├── Smaller total file size for 3+ weights
└── Width, slant, and custom axes available
```
...
/* Use any weight precisely */
.body { font-weight: 380; }     /* Between Light and Regular */
.emphasis { font-weight: 520; } /* Between Medium and SemiBold */
.heading { font-weight: 750; }  /* Between Bold and ExtraBold */
...
### Recommended Variable Fonts (Anti-Slop)

| Font | Axes | Style | Source |
|------|------|-------|--------|
| **Fraunces** | wght, opsz, SOFT, WONK | Display serif, quirky | Google Fonts |
| **Satoshi** | wght | Modern geometric sans | Fontshare |
| **Space Grotesk** | wght | Technical, distinctive | Google Fonts |
| **Instrument Serif** | ital | Elegant editorial serif | Google Fonts |
| **DM Sans** | wght, ital, opsz | Clean humanist sans | Google Fonts |
| **Sora** | wght | Geometric, futuristic | Google Fonts |

### When to Use Variable vs Static

| Scenario | Recommendation |
|----------|---------------|
| Using 3+ weights | ✅ Variable (smaller total size) |
| Weight animation needed | ✅ Variable (only option) |
| Single weight only | Static (smaller single file) |
| Critical performance | Test both (measure actual sizes) |

---

## 11. Fluid Spacing System

### Beyond Fixed `rem` Values

Just as fluid typography uses `clamp()` for font sizes, fluid spacing creates dramatic variation between sections at different viewport sizes — a key technique for avoiding the rigid, formulaic feel of AI output.

```css
/* Fixed spacing (❌ rigid, same on mobile and desktop) */
padding: 4rem;

/* Fluid spacing (✅ responsive, dramatic variation) */
padding: clamp(2rem, 5vw + 1rem, 8rem);
/*        MIN     PREFERRED      MAX  */
```
...
:root {
  ...
  --space-lg:  clamp(1.5rem, 4vw + 0.5rem, 4rem);
  --space-xl:  clamp(2rem, 6vw + 1rem, 8rem);
  --space-2xl: clamp(4rem, 10vw + 2rem, 14rem);
  --space-section: clamp(3rem, 8vw + 2rem, 12rem);
}

/* Section spacing creates dramatic breathing room on desktop */
.section { padding-block: var(--space-section); }

/* Component spacing stays comfortable at all sizes */
.card { padding: var(--space-md); gap: var(--space-sm); }
```

### The Key Insight

Fluid spacing is most impactful for **section-level padding** where you want:
- Mobile: Compact, space-efficient (3rem padding)
- Desktop: Generous, editorial breathing room (12rem+ padding)

This variation makes layouts feel intentionally designed rather than uniformly computed.

---

## 12. Typography Selection Checklist
