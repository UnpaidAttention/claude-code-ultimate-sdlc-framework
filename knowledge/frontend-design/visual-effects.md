...
├── ⚠️ Use sparingly and ONLY with custom color stops (avoid generic aurora/blob patterns)
...
## 8. SVG Grain & Noise Textures

### Why Grain Matters

Grain/noise textures add organic atmosphere that breaks the "too clean" AI look. They're a key differentiator between generic flat output and designs with tactile character.

### The Pattern (SVG feTurbulence)

```html
<!-- Inline SVG noise filter — add once, reuse via CSS -->
...
```css
/* Apply as overlay with CSS pseudo-element */
.textured-section::after {
  content: '';
  position: absolute;
  inset: 0;
  filter: url(#grain);
  opacity: 0.08;           /* Subtle: 0.03-0.08, Bold: 0.10-0.15 */
...
| Parameter | Low (Subtle) | Medium | High (Bold) |
|-----------|-------------|--------|-------------|
| `baseFrequency` | 0.5-0.6 | 0.65 | 0.7-0.8 |
| `numOctaves` | 2 | 3 | 4 |
| `opacity` | 0.03-0.05 | 0.06-0.08 | 0.10-0.15 |

### When to Use

- ✅ Hero sections and large background areas (adds depth)
- ✅ Dark mode surfaces (reduces "screen glow" feel)
- ✅ Gradient backgrounds (softens banding artifacts)
- ✅ Photography overlays (unifies mixed image styles)
- ❌ Over text-heavy content (hurts readability)
- ❌ On small UI elements like buttons (too noisy)

### Performance Note

SVG filters are GPU-accelerated. For very large areas, consider using a pre-generated noise PNG as `background-image` with `repeat` instead of real-time SVG filtering.
...
@layer tokens {
  :root { --color-primary: oklch(0.55 0.18 250); }
}
...
**Why layers matter**: They replace specificity battles and `!important` hacks. Tailwind v4 uses layers by default.
