...
> **Full reference**: See `animation-guide.md` Section 10 for comprehensive CSS scroll-driven animation patterns (`scroll()`, `view()`, `animation-range`), code examples, and decision table (CSS vs JS).

Quick decision: Use CSS `animation-timeline: view()` for simple reveals/parallax. Use JS (GSAP/Framer) for staggered sequences, physics-based motion, or complex timelines.
